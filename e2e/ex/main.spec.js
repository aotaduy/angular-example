...
describe('Expenses app', function () {
    var page;
    beforeEach(function () {
        browser.get('/demo/ex/assignment2/');
        page = {};
    });
    ...
    it('should add/edit/delete an item from/to the table', function () {
        page.date = element(by.model('exp.expense.date'));
        page.date.sendKeys('09/06/1988');

        page.type = element(by.cssContainingText('option', 'transportation')).click();
        page.description = element(by.model('exp.expense.description')).sendKeys('Testing Expense');
        page.amount = element(by.model('exp.expense.amount')).sendKeys('1000');

        page.button = element(by.id('create-button'));
        page.button.click();

        //Quantity
        var items = element.all(by.repeater('expen in exp.expenses track by expen.id'));
        ...
        expect(element.all(by.repeater('expen in exp.expenses track by expen.id')).count()).to.eventually.equal(0);
    });
